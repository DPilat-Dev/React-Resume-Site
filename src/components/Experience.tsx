  ...
  return (
    <div
      ...
      className={`cursor-pointer sm:mb-5 p-5 max-w-xl relative sm:text-left text-center ${
        isMobile ? "text-sky-700" : ""
      }`}
    >
      {(isActive || isMobile) && (
        <div className="absolute left-0 top-0 bottom-0 w-3 md:w-5 bg-sky-700 my-6 sm:block hidden"></div>
      )}
      <h3
        className={`text-xl lg:text-2xl xl:text-3xl font-bold sm:pl-8 ${
          isActive || isMobile ? "text-sky-700" : "text-slate-600"
        }`}
      >
        {experience.title}
      </h3>
...
const ExperienceDetails = ({experience}:ExperienceDetailProps) => {
  return (
    <div className="mt-5">
...
const Experience = () => {
  ...
  return (
    <div className="sm:my-20">
      ...
      <div className="relative mt-10 md:mt-20 md:p-20 flex flex-col items-center sm:flex-row sm:item-start">
