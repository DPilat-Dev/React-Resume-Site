...
const DynamicIcon: React.FC<DynamicIconProps> = ({ iconName, size = 24, color = "white" }) => {
  ...
  useEffect(() => {
    const loadIcon = async () => {
      try {
        ...
        const module = await iconLibraries[libKey]();
        const LoadedIcon = module[iconName as keyof typeof module];
